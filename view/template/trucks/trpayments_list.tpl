<table id="listMasterTable" class="mtable table table-striped table-bordered table-responsive table-condensed table-hover" style="width:100%">
    ...
    <tbody>
        <?php if ($items) {
            ...
            <tr id="list_<?php echo $item['id']; ?>" <?php if($item['id_transporter_payment']){ echo 'class="success"';}?> >
                <td><?php 
	if(!$item['id_transporter_payment']){		
	...
	?></td>
		    ...
		    <td id="list_material_<?php echo $item['id']; ?>"><?php echo $item['material']; ?></td>
                    <td id="list_transtype_<?php echo $item['id']; ?>"><?php echo $item['transtype']; ?></td>
		    <td id="list_transactiondate_<?php echo $item['id']; ?>"><?php echo date(FORMAT_DATETIME_FORMAT, strtotime($item['transactiondate'])); ?></td>
		    <td id="list_dispatchdate_<?php echo $item['id']; ?>"><?php echo date(FORMAT_DATE_FORMAT, strtotime($item['dispatchdate'])); ?>
		    ...
		    <td id="list_unloadingexp_<?php echo $item['id']; ?>"><?php echo $item['unloadingexp']; ?></td>
                    <td id="list_transporteradvance_<?php echo $item['id']; ?>"><?php echo $item['transporteradvance']; ?></td>
                    <td id="list_podamount_<?php echo $item['id']; ?>"><?php echo ($item['qty']*$item['truckrate'])-($item['shortage']+$item['damage']+$item['loadingexp']+$item['unloadingexp']+$item['transporteradvance']); ?></td>
                    <td id="list_ispodreceived_<?php echo $item['id']; ?>"><?php echo $iconCheck[$item['ispodreceived']]; ?></td>
		    <td id="list_ispodsubmitted_<?php echo $item['id']; ?>"><?php echo $iconCheck[$item['ispodsubmitted']]; ?></td>
                    <td id="list_id_transporter_payment_<?php echo $item['id']; ?>">
		    <?php if($item['id_transporter_payment']){ echo '<button type="button" class="btn btn-sm btn-primary" onclick="fnTrPayment('.$item['id_transporter_payment'].')" >'.$item['id_transporter_payment'].'</button>';} ?></td>
...
$("input[type='checkbox']").on("click",function(){
	...
	$('input:checked').each(function() {
		...
		nooftrips+=1;
                //alert(jsn['freight']+" - "+jsn['transporteradvance']+" "+jsn['shortage']+" "+jsn['damage']);
		amount+=parseFloat(jsn['qty']*jsn['truckrate'])-(parseFloat(jsn['transporteradvance'])+parseFloat(jsn['shortage'])+parseFloat(jsn['damage'])+parseFloat(jsn['loadingexp'])+parseFloat(jsn['unloadingexp']));
	});
        //alert(amount)
	$('#field_nooftrips').val(nooftrips);
	$('#field_totalreceivableamount').val(amount);
