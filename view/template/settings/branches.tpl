...
<script>
    function fnSubmit(btn) {
        ...
        if ($('#field_branchcode').val() == "") {
            //alert("inside");
            $('#field_branchcode').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_branchcode').closest('.col-md-8').removeClass('has-error');
        }

        if ($('#field_branchcity').val() == "") {
            //alert("inside");
            $('#field_branchcity').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_branchcity').closest('.col-md-8').removeClass('has-error');
        }

	 if ($('#field_branchcontactname').val() == "") {
            //alert("inside");
            $('#field_branchcontactname').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_branchcontactname').closest('.col-md-8').removeClass('has-error');
        }

	if ($('#field_branchcontactmobile').val() == "") {
            //alert("inside");
            $('#field_branchcontactmobile').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_branchcontactmobile').closest('.col-md-8').removeClass('has-error');
        }
        ...
        if (flag) {
            ...
            $.ajax({
				...
				var isheadofficeHtml=parseJsonData['isheadoffice']=='1'?'<i class="fa fa-check" aria-hidden="true"></i>':'<i class="fa fa-times" aria-hidden="true"></i>';
				$('#list_isheadoffice_' + type).html(isheadofficeHtml);
