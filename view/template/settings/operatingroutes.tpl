...
<div id="page-wrapper">
    ...
    <div class="row" >
        ...
        <?php include 'operatingroutes_form.tpl';?>
        <!-- /.row -->
    </div>
...
<script>
    function fnSubmit(btn) {
        ...
        if ($('#field_operatingroutecode').val() == "") {
            //alert("inside");
            $('#field_operatingroutecode').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_operatingroutecode').closest('.col-md-8').removeClass('has-error');
        }

        if ($('#field_from').val() == "") {
            //alert("inside");
            $('#field_from').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_from').closest('.col-md-8').removeClass('has-error');
        }

	 if ($('#field_to').val() == "") {
            //alert("inside");
            $('#field_to').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_to').closest('.col-md-8').removeClass('has-error');
        }

	if ($('#field_distance').val() == "") {
            //alert("inside");
            $('#field_distance').closest('.col-md-8').addClass('has-error');
            var flag = 0;
        } else {
            $('#field_distance').closest('.col-md-8').removeClass('has-error');
        }
