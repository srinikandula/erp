...
<div class="form-group col-md-2 required">
    <label for="accountID" class="col-md-6">Transporter :</label>
    <div class="col-md-6">
        <select name="filter_id_transporter" class="form-control" title="Transporter" id="field_id_transporter" >
        <option value="">All Transporters</option>    
        <?php foreach($transporters as $row){
            echo '<option value="'.$row['id_transporter'].'">'.$row['transporter'].'</option>';
        }?>
        </select>
    </div>
</div>
...
<div class="form-group col-md-2 required">
    <label for="accountID" class="col-md-8">Bill Raised</label>
    <div class="col-md-4">
        <!-- <input type="checkbox" name="filter_ispodsubmitted" class="form-control"  title="POD Submitted" id="field_ispodsubmitted" > -->
	<select name="filter_trbillraised" class="form-control"  title="Bill Raised" id="field_trbillraised" >
        <option value="">All</option>    
	<option value="1">Yes</option>    
	<option value="0">No</option>    
        </select>
...
function fnDownload(){
	location.href='index.php?route=reports/trpod/download&token=<?php echo $_GET["token"];?>&'+$('form#horizontalSearchFormFilter').serialize();
}
</script>