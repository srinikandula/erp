...
<div class="search-container search-row col-md-12">
	...
	<fieldset class="large-form-fieldset">
        <legend>Filter <div style="float:right;padding:0px 2px 2px; 10px"><span class="btn-success btn-xs">Factory Payment Closed</span> <span class="btn-warning btn-xs">TR Pending Bill</span> <span class="btn-danger btn-xs">O/S Trips</span> <button type="button" class="btn btn-info btn-xs" onclick="fnDownload()"><i class="fa fa-download" aria-hidden="true"></i> Download</button> <!-- <button type="button" class="btn btn-info btn-xs"><i class="fa fa-print" aria-hidden="true"></i> Print</button> --></div></legend>
...
<div class="form-group col-md-2">
    ...
    <div class="col-md-2">
        <input name="filter_factory_os"  placeholder="Factory O/S Trips" data-toggle="tooltip" data-placement="auto top" title="" id="filter_factory_os"  data-original-title="Factory O/S Trips" type="checkbox">
    </div>
</div>

<div class="form-group col-md-2">
    <label for="accountID" class="col-md-6">In/Out :</label>
    <div class="col-md-6">
        <select name="filter_own" class="form-control" title="Tuck" id="filter_own" >
...
function fnDownload(){
	...
	location.href='index.php?route=reports/trips/download&token=<?php echo $_GET["token"];?>&'+$('form#horizontalSearchFormFilter').serialize();
}
</script>