...
class ControllerSettingsBranches extends Controller {

    private $error = array();
    
    public function index() {

        //$this->document->setTitle('Branches');
	...
	$data['header'] = $this->load->controller('common/header');
        ...
        $this->response->setOutput($this->load->view('settings/branches', $data));
    }
	...
	//$filter_materialcode=isset($this->request->get['filter_materialcode'])?$this->request->get['filter_materialcode']:'';
        ...
        $itemTotal = $this->model_settings_branch->getTotalItems($filter_data);
        //exit(" value of ");
        $results = $this->model_settings_branch->getItems($filter_data);
	...
	//echo '<pre>';print_r($results->rows);
        ...
        foreach ($results->rows as $k=>$result) {
            $data['items'][] = array(
                ...
                'branchcity' => $result['branchcity'],
                'branchaddress' => $result['branchaddress'],
                'branchcontactname' => $result['branchcontactname'],
                'branchcontactmobile' => $result['branchcontactmobile'],
                'isheadoffice' => $result['isheadoffice'],
                'datecreated' => $result['datecreated'],
                'datemodified' => $result['datemodified'],
                'edit' => $this->url->link($route . '/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id_branch'] . $buttonUrl, true),
				'sno'=>$k+$pageSno,
            );
        }
	...
	$sortUrl=getUrlString($urlData,$urlDataIgnore);
        ...
        $data['sort_branchcity'] = $this->url->link($route, 'token=' . $this->session->data['token'] . '&sort=branchcity' . $sortUrl, true);
        $data['sort_branchcontactname'] = $this->url->link($route, 'token=' . $this->session->data['token'] . '&sort=branchcontactname' . $sortUrl, true);
        $data['sort_branchcontactmobile'] = $this->url->link($route, 'token=' . $this->session->data['token'] . '&sort=branchcontactmobile' . $sortUrl, true);
        
        $data['selected']=isset($this->request->post['selected'])?(array) $this->request->post['selected']:array();	
	
        $this->response->setOutput($this->load->view('settings/branches_list', $data));
    }
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm(array('perm' => 'add'))) {
            ...
            $this->model_settings_branch->addItem($cols);
            $json['status'] = 1;
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm(array('perm' => 'edit'))) {
            ...
            $cols=$this->request->post['field'];
            $cols['datemodified']=$this->globalVal['todayDateTime'];
            $this->model_settings_branch->editItem($this->request->post['pkey'], $cols);
            $json['status'] = 1;
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete(array('perm' => 'delete'))) {
            ...
            foreach ($this->request->post['selected'] as $id) {
                $this->model_settings_branch->deleteItem($id);
            }
            $json['status'] = 1;
    ...
    protected function validateForm($input) {
        ...
        if (!$this->user->hasPermission($input['perm'], 'settings/branches')) {
