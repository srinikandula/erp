...
class ControllerInventoryPartsin extends Controller {

    private $error = array();
    public $viewPathPrefix='inventory/partsin';	
	...
	$data['header'] = $this->load->controller('common/header');
        ...
        $data['storeVendors'] =$storeVendorsQry->rows;
        /*foreach($storeVendorsQry->rows as $row){
            $data['storeVendors'][$row['type']][] =$row;
        }*/
    ...
    public function getlist() {
        ...
        $filter_id_store_vendor=isset($this->request->get['filter_id_store_vendor'])?$this->request->get['filter_id_store_vendor']:'';
        $filter_id_branch_store=isset($this->request->get['filter_id_branch_store'])?$this->request->get['filter_id_branch_store']:'';
	...
	//$filter_materialcode=isset($this->request->get['filter_materialcode'])?$this->request->get['filter_materialcode']:'';
        ...
        $urlData['filter_id_store_vendor']=$filter_id_store_vendor;
        $urlData['filter_id_branch_store']=$filter_id_branch_store;
        ...
        $filter_data = array(
            'filter_id_store_vendor' => $filter_id_store_vendor,
            'filter_id_branch_store' => $filter_id_branch_store,
        ...
        $itemTotal = $this->model_settings_storevendoritem->getTotalItemsByVendorBranch($filter_data);

        $results = $this->model_settings_storevendoritem->getItemsByVendorBranch($filter_data);
		...
		foreach ($results->rows as $k=>$result) {
            $data['items'][$k] =$result;
            $data['items'][$k]['id']=$result['id_store_vendor_item'];
			$data['items'][$k]['type']=$storeItems[$result['type']];
            $data['items'][$k]['edit']=$this->url->link($route . '/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id_store_vendor_item'] . $buttonUrl, true);
    ...
    protected function validateForm($input) {
        ...
        if (!$this->user->hasPermission($input['perm'], 'inventory/partsin')) {
