...
class ControllerTrucksTrpayments extends Controller {

    private $error = array();
    public $viewPathPrefix='trucks/trpayments';	
    public function index() {

        //$this->document->setTitle('Transporter Payments');
    ...
    public function getPaymentDetails() {
        ...
        $json['status']=0;
        if($id){
            $this->load->model('settings/transporterpayment');
            $payData = $this->model_settings_transporterpayment->getItem($id);
            $json['status']=1;
            $json['truck']=$payData;
            
            $this->load->model('settings/trip');
            $truckData = $this->model_settings_trip->getTripsByTransporterPaymentID($id);
	...
	//$filter_materialcode=isset($this->request->get['filter_materialcode'])?$this->request->get['filter_materialcode']:'';
        ...
        $filter_data = array(
            ...
            'filter_loadprovider' => $filter_loadprovider,
            'sort' => $data['sort'],
            'order' => $data['order'],
	...
	//$filter_materialcode=isset($this->request->get['filter_materialcode'])?$this->request->get['filter_materialcode']:'';
        ...
        //echo '<pre>';print_r($filter_data);echo '</pre>';
        $itemTotal = $this->model_settings_transporterpayment->getTotalItems($filter_data);

        $results = $this->model_settings_transporterpayment->getItems($filter_data);
        $pageSno = !isset($this->request->get['page']) ? 1 : ($this->request->get['page'] * $this->config->get('config_limit_admin')) - 1;
        foreach ($results->rows as $k=>$result) {
            $data['items'][$k] =$result;
            $data['items'][$k]['id']=$result['id_transporter_payment'];
            $data['items'][$k]['edit']=$this->url->link($route . '/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id_transporter_payment'] . $buttonUrl, true);
            $data['items'][$k]['sno']=$k+$pageSno;
        }
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm(array('perm' => 'add'))) {
            ...
            $id=$this->model_settings_transporterpayment->addItem(array('chequeno' => $field['chequeno'],'nooftrips' => $field['nooftrips'],'totalreceivableamount' => $field['totalreceivableamount'],'id_branch' => $field['id_branch'],'paymentmode' => $field['paymentmode'],'datereceived' => $field['datereceived'],'id_transporter' => $this->request->post['filter_id_transporter'], 'datecreated' => $this->globalVal['todayDateTime']));
            
            $this->load->model('settings/trip');
            $TrTrips=explode(",",$this->request->post['field_trips']);
            foreach($TrTrips as $id_tfs){
                $this->model_settings_trip->editItem($id_tfs,array('id_transporter_payment' => $id));
            }
            $json['status'] = 1;
