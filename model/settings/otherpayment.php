...
class ModelSettingsOtherpayment extends Model {
	...
	public function getItemsTrans($data = array()) {
		...
		if($data['filter_id_branch']!=""){
			...
			$sql8=" and id_branch=".(int)$data['filter_id_branch'];
		}
		...
		union all
		(select 'Dr' as transtype,concat('Payment Made to Store  for ID #',svp.	id_store_vendor_payment) as particulars,concat('Pay Mode: ',svp.paymentmode,' Pay ref: ', svp.paymentreference,' ',svp.comment) as narration ,svp.id_branch_payment as id_branch,svp.datepurchased as transdate,svp.amount from ".DB_PREFIX."store_vendor_payment svp where svp.datepurchased>='".$data['filter_fromdate']."' and  svp.datepurchased<='".$data['filter_todate']."'".$sql7.")
		union all
		(select transactiontype as transtype,particulars,concat('Pay Mode: ',paymentmode,' Pay ref: ', paymentref, ' ',narration) as narration,id_branch, transactiondate as transdate,amount from ".DB_PREFIX."other_payment where transactiondate>='".$data['filter_fromdate']."' and  transactiondate<='".$data['filter_todate']."'".$sql8.")

		) as drtable order by transdate desc";
