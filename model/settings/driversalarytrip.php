<?php
class ModelSettingsDriversalarytrip extends Model {
	public $mTable;
	public $pKey;

	public function __construct($className=__CLASS__){
		$this->mTable=DB_PREFIX.'driver_salary_trip';
		$this->pKey='id_driver_salary_trip';
	...
	public function deleteItemByDriverSalaryID($id) {
		$this->db->query("DELETE FROM ".$this->mTable." WHERE id_driver_salary = '" . (int)$id . "'");
	}
	...
	public function getFilterSql($data,$sql){
		...
		if (!empty($data['filter_id_driver_salary'])) {
			$sql .= " AND id_driver_salary='" . (int)$data['filter_id_driver_salary'] . "'";
		}
