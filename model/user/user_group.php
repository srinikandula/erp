...
class ModelUserUserGroup extends Model {
	...
	public function getPermissions(){
		$return = $this->cache->get('getPermissions');
		if (!$return) {
			...
			foreach($user_permissions_query->rows as $row){
				$return[$row['modulename']][$row['filename']]=array('title'=>$row['title'],'link'=>$this->url->link(strtolower(str_replace(" ","",$row['modulename']."/".$row['filename'])), 'token=' . $this->session->data['token'], true),'listall'=>$row['listall'],'view'=>$row['view'],'add'=>$row['add'],'edit'=>$row['edit'],'delete'=>$row['delete']);
			}
			$this->cache->set('getPermissions', $return);
		}
